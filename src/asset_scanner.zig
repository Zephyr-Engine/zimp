...
pub const AssetScanner = struct {
    ...
    pub fn scan(self: AssetScanner) !std.ArrayList(SourceFile) {
        var files: std.ArrayList(SourceFile) = .empty;
        try self.scanDir(self.dir, &files);
        return files;
    }

    fn scanDir(self: AssetScanner, dir: std.Io.Dir, files: *std.ArrayList(SourceFile)) !void {
        var iter = dir.iterate();
        while (try iter.next(self.io)) |entry| {
            if (entry.kind == .file) {
                try files.append(self.allocator, .{
                    .extension = Extension.gltf,
                    .path = entry.name,
                    .size = 0,
                });
